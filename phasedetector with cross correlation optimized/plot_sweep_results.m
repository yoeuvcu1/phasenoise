...
% Dekad MAE etiketleri eğrinin biraz üstüne yazılır; üst sınır bu etiketleri
% kırpmayacak kadar açılır.
label_offset = max(2, 0.05*(level_max - level_min));
level_headroom = level_padding + 1.6*label_offset;
...
for value_index = 1:number_of_values
    ...
    ylim(ax, [level_min - level_padding, level_max + level_headroom]);
    ...
    title(ax, {sprintf("%s | %s", sweep_name, current_label), ...
        config_details, run_details, ...
        "numbers above curves: decade MAE (dB)"}, "Interpreter", "none");

    % Başlıktaki MAE bütün offset bölgesini tek sayıya indirir; LPF geçiş
    % bandındaki uyum ile kesim üstündeki artık ayrışma aynı ortalamada
    % karışır. Dekad bazlı MAE aynı farkı offset bölgelerine ayırır ve her
    % değer ait olduğu dekadın ortasında, eğrinin üzerine yazılır. Log eksende
    % dekad merkezleri eşit aralıklı olduğundan etiketler de eşit aralıklıdır.
    bands = decade_band_errors( ...
        current_results.cross.frequency_binned, ...
        current_results.cross.phase_noise_binned, ...
        dut_plot.frequency_binned, dut_plot.phase_noise_binned, []);
    for band_index = 1:numel(bands)
        band_center = sqrt(bands(band_index).frequency_low * ...
            bands(band_index).frequency_high);
        % Etiket, o frekanstaki iki eğriden yüksek olanın üzerine yerleşir.
        level_cross_here = interp1( ...
            log10(current_results.cross.frequency_binned), ...
            current_results.cross.phase_noise_binned, ...
            log10(band_center), "linear", "extrap");
        level_dut_here = interp1( ...
            log10(dut_plot.frequency_binned), ...
            dut_plot.phase_noise_binned, ...
            log10(band_center), "linear", "extrap");
        label_level = max(level_cross_here, level_dut_here) + label_offset;
        text(band_center, label_level, ...
            sprintf("%.2f", bands(band_index).mean_absolute_error_db), ...
            "Parent", ax, ...

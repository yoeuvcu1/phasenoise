...
%   7. Secilen son iterasyondaki Cross-PSD / DUT periodogrami karsilastirmasi
%
% Bu dosya, matlab_version/presentation_signal_flow_plots klasorunun
% GNU Octave uyumlu kopyasidir. Dosya adiyla ayni ana fonksiyon ve ayni
% dosyada kullanilan alt fonksiyonlardan olusur.

function generate_signal_flow_plots()
script_dir = fileparts(mfilename("fullpath"));
generate_signal_flow_plots_main(script_dir);
end
...
if ~isempty(cutoff)
    % Octave'de xline olmadigi icin dikey kesikli cizgi line/text ile cizilir.
    axis_limits = ylim(ax);
    cutoff_x = cutoff.frequency/1e3;
    line([cutoff_x, cutoff_x], axis_limits, ...
        "LineStyle", "--", "Color", cutoff.color, "LineWidth", 1.4);
    text(ax, cutoff_x + 0.01*diff(xlim(ax)), mean(axis_limits), ...
        cutoff.label, "Interpreter", "tex", "Color", cutoff.color, ...
...
function fig = standard_figure(figure_name, figure_width, figure_height)
% Varsayilan 7x6 kareye yakin oran; genis zaman domeni grafigi icin
% en ve boy cagiran taraftan verilir.
if nargin < 2
    figure_width = 7;
    figure_height = 6;
elseif nargin < 3
...
fig = figure("Name", figure_name, "Visible", "off", "Color", "white", ...
    "Units", "inches", "Position", [1, 1, figure_width, figure_height]);
end

function style_axis(ax)
grid(ax, "on");
set(ax, "GridAlpha", 0.20, "MinorGridAlpha", 0.10, ...
    "FontName", "Helvetica", "FontSize", 11, ...
    "LineWidth", 0.8, "Box", "on");
end

function export_figure(fig, output_path)
% Figure'in inch boyutunu 150 DPI ile PNG olarak kaydeder.
figure_size = get(fig, "Position");
set(fig, "PaperUnits", "inches");
set(fig, "PaperPosition", [0, 0, figure_size(3), figure_size(4)]);
set(fig, "PaperSize", [figure_size(3), figure_size(4)]);
...
function generate_signal_flow_plots_main(script_dir)
...
if exist("OCTAVE_VERSION", "builtin")
    % Butterworth tasarimi Octave'de signal paketinden gelir.
    pkg load signal;
end
...
config.phase_rms_ref1 = 0.1;
config.phase_rms_ref2 = 0.1;
...
correlation_final = struct();

%% ---------------- TWO-CHANNEL ITERATION FLOW ----------------
fprintf("Sunum grafikleri icin %d iterasyon hesaplaniyor...\n", ...
    config.number_of_iterations);
simulation_timer = tic;
progress_interval = max(1, ceil(config.number_of_iterations/5));
...
for iteration = 1:config.number_of_iterations
    ...
    if iteration == config.number_of_iterations
        correlation_final = build_correlation_result( ...
            f_cross, S_cross_sum, S_dut_sum, iteration, ...
            config.number_of_log_bins);
    end

    if mod(iteration, progress_interval) == 0 || ...
            iteration == config.number_of_iterations
        fprintf("  %d/%d iterasyon tamamlandi.\n", ...
            iteration, config.number_of_iterations);
    end
end
...
if isempty(fieldnames(correlation_1)) || isempty(fieldnames(correlation_final))
    error("1 ve son iterasyon kontrol noktalari olusturulamadi.");
end
...
fig = standard_figure("DUT ve Referans Sinyalleri", 10, 6);
ax = axes(fig);
...
plot_correlation_comparison( ...
    correlation_final, sprintf("Korelasyon Sonucu - %d İterasyon", ...
    config.number_of_iterations), ...
    blue, red, fullfile(output_dir, ...
    sprintf("07_korelasyon_%d_iterasyon.png", ...
    config.number_of_iterations)));
...
fprintf(summary_file, "Presentation FFT plots (Octave)\n");
...
fprintf(summary_file, "iterations=1,%d\n", config.number_of_iterations);
